...
REAL inthelperf_bkmom(REAL ktsqr, void* p)
{
    inthelper_bkmom* par = (inthelper_bkmom*) p;

    REAL result=0;

    // For very small kt amplitude -> N(ksqr')/ksqr
    ///TODO: Check
    //if (ktsqr < 0.0001 and ktsqr > par->ktsqr*10.0)
    //    return par->N->N(ktsqr,par->y)/par->ktsqr;
    
    // When ktsqr -> par->ktsqr first doesn't diverge, but numerics would fail
    // as there is term 1/(ktsqr - par->ktsqr)

  /*  if (abs(ktsqr - par->ktsqr) < 0.0000001)
    {
    ...
    result += par->ktsqr * par->N->N(par->ktsqr, par->y) / sqrt( 4*SQR(ktsqr) + SQR(par->ktsqr));
...
REAL Amplitude::RapidityDerivative(REAL ktsqr, REAL y)
{
    ...
    int_helper.function=&inthelperf_bkmom;
